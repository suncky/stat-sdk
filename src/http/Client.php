...
final class Client
{
    public static function get($url, $data = array(), array $headers = array())
    {
        $url = Func::format_get($url, $data);
    ...
    public static function multipartPost(
        ...
        $fileName = self::_escapeQuotes($fileName);
    ...
    private static function _sendRequest(Request $request)
    {
        $t1 = microtime(true);
        $ch = curl_init();
        $options = array(
        ...
        $headers = self::_parseHeaders(substr($result, 0, $header_size));
        $body = substr($result, $header_size);
        curl_close($ch);
        return new Response($code, $duration, $headers, $body, null);
    }

    private static function _parseHeaders($raw)
    {
        $headers = array();
        $headerLines = explode("\r\n", $raw);
    ...
    private static function _escapeQuotes($str)
    {
        $find = array("\\", "\"");
        $replace = array("\\\\", "\\\"");
